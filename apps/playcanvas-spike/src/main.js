...
import {
  applyApprovedTopdownCamera,
  APPROVED_TOPDOWN_CAMERA,
} from "./camera-contract.js";
import {
  rendererFailureMessage,
  supportsRequiredWebGL,
...
const rendererActions = document.querySelector("#renderer-actions");
const retryRenderer = document.querySelector("#retry-renderer");

function showRendererFailure(reason) {
  loading.classList.remove("is-hidden");
  loading.classList.add("is-error");
  loadingMessage.textContent = rendererFailureMessage(reason);
  rendererActions.hidden = false;
  statusPill.textContent = "3D-РЕНДЕР НЕДОСТУПЕН";
}

retryRenderer.addEventListener("click", () => window.location.reload());

if (!supportsRequiredWebGL(document.createElement("canvas"))) {
  showRendererFailure("unavailable");
  throw new Error("DOFFA Heroes requires WebGL2 for the true-3D route");
}
...
camera.addComponent("camera", {
  ...
  orthoHeight: APPROVED_TOPDOWN_CAMERA.orthoHeight,
  clearColor: new Color(0.012, 0.009, 0.006),
  nearClip: 0.1,
  farClip: 100,
});
applyApprovedTopdownCamera(camera);
...
app.on("update", (dt) => {
  ...
  camera.camera.orthoHeight = APPROVED_TOPDOWN_CAMERA.orthoHeight;
});
