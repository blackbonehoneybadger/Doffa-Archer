...
Для Unity Personal файл `.ulf` нужно один раз получить через **Unity Hub → Licenses → Add license → Get a free personal license**. Unity 6.3 больше не разрешает ручной обмен `.alf` через веб-портал для Personal.
